...
void Semaphore::processActions() {
    ...
    while (it != pendingActions.end()) {
        ...
        std::cerr << "Debug [Semaphore]: Processing action " << action.getProcessId()
        ...
        if (!resource) { // Recurso no valido
            std::cerr << "Debug [Semaphore]: Invalid resource for action " << action.getProcessId() << std::endl;
            ++it;
            continue;
        }
        ...
        if (action.getStatus() == ActionStatus::ACCESSED) {
            std::cerr << "Debug [Semaphore]: Completing action " << action.getProcessId() << std::endl;
        ...
        if (action.getStatus() == ActionStatus::WAITING) {
            std::cerr << "Debug [Semaphore]: Attempting to acquire resource for action " << action.getProcessId() << std::endl;
            bool acquired = resource->acquire(action.getProcessId()); // Intentar adquirir (maneja cola)

            if (acquired) {
                std::cerr << "Debug [Semaphore]: Acquired resource for action " << action.getProcessId() << std::endl;
                ...
                // Sigue esperando
                std::cerr << "Debug [Semaphore]: Could not acquire resource for action " << action.getProcessId() << ". Still waiting." << std::endl;
        ...
        // O un estado inesperado. La tratamos como WAITING.
         std::cerr << "Debug [Semaphore]: Unexpected action status for " << action.getProcessId() << ". Treating as WAITING." << std::endl;
         bool acquired = resource->acquire(action.getProcessId());
        if (acquired) {
             std::cerr << "Debug [Semaphore]: Acquired resource (fallback) for action " << action.getProcessId() << std::endl;
             action.setStatus(ActionStatus::ACCESSED);
             updateTimeline(action.getProcessId(), ActionStatus::ACCESSED);
             ++it;
        } else {
             std::cerr << "Debug [Semaphore]: Could not acquire resource (fallback) for action " << action.getProcessId() << ". Still waiting." << std::endl;
    ...
    completedActions.insert(completedActions.end(), justCompletedActions.begin(), justCompletedActions.end());
     std::cerr << "Debug [Semaphore]: ProcessActions finished. Total completed: " << completedActions.size() << ", Total actions: " << actions.size() << ", Pending actions: " << pendingActions.size() << std::endl;
